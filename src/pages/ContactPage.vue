...
const channels: ContactChannel[] = [
  ...
  {
    title: "GitHub",
    description: "Подходит для технических вопросов, задач по проектам, GitHub Issues и предложений по улучшению.",
    bestFor: "технические детали, задачи и публичное обсуждение",
...
const workFormats: string[] = [
  "Точечная консультация по backend-архитектуре и API",
  "Ревью и оздоровление существующего серверного кода",
  "Проектирование сервисов, интеграций, очередей и работы с данными",
  "Подключение к проекту в роли Backend Node.js Developer"
];

const collaborationSignals: string[] = [
  "Нужно навести порядок в структуре проекта",
  "Есть продуктовая идея, которую пора превратить в рабочий backend или API",
  "Нужен спокойный техразбор перед реализацией",
  "Хочется улучшить надёжность, DX команды и убрать лишнюю хрупкость"
];
...
<template>
  <section class="page-header reveal home-hero contact-hero">
    ...
    <p class="page-lead">
      Готов обсудить Node.js backend, архитектуру информационных систем,
      техническую консультацию или подключение к продуктовой команде.
    </p>
    ...
    <div class="contact-card-grid">
      <article v-for="channel in channels" :key="channel.title" class="contact-card">
        ...
        <a :href="channel.href" target="_blank" rel="noreferrer noopener" class="stack-chip contact-card-link">
          ...
          <svg
            v-else
            class="stack-chip-logo social-logo social-logo-telegram telegram-icon"
            viewBox="0 0 24 24"
            aria-hidden="true"
          >
            <path
